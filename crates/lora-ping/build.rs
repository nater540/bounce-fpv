...
// add OUT_DIR to the linker search path so link.x (pulled in by -Tlink.x in .cargo/config.toml) can INCLUDE it. The
// `-Tdefmt.x` script lora-phy's unconditional defmt dependency needs is now added globally in .cargo/config.toml (it
// pairs with applog's defmt-rtt #[global_logger]), so it is no longer handled per-binary here.
fn main() {
  let out = PathBuf::from(env::var("OUT_DIR").unwrap());
  // memory.x lives at the workspace root, two levels up from this crate (crates/lora-ping/).
